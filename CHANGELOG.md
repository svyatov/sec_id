...
- `SecID::CFI#decode` returns a frozen `SecID::CFI::Classification` value object for any valid CFI (`nil` for invalid) — exposing the category and group as symbols, the four attributes decoded and keyed by each position's group-specific meaning, the authoritative ISO 10962 label strings, a convenience predicate per attribute value (e.g. `voting?`, `fully_paid?`, `registered?`), and a human-readable `#to_s`. `X` decodes to `:not_applicable`; pure-N/A positions are omitted
