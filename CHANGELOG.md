...
- `SecID::CFI#decode` returns a frozen `SecID::CFI::Classification` value object for any valid CFI (`nil` for invalid) — exposing the category and group as symbols, the four attributes decoded and keyed by each position's group-specific meaning, the authoritative ISO 10962 label strings, a convenience predicate per attribute value (e.g. `voting?`, `fully_paid?`, `registered?`), and a human-readable `#to_s`, and `#to_h`/`#as_json` for serialization. `X` decodes to `:not_applicable`; pure-N/A positions are omitted
...
- **BREAKING:** the 12 hardcoded equity predicate helpers on `SecID::CFI` (`equity?`, `voting?`, `non_voting?`, `restricted_voting?`, `enhanced_voting?`, `restrictions?`, `no_restrictions?`, `fully_paid?`, `nil_paid?`, `partly_paid?`, `bearer?`, `registered?`). They were category-wide and semantically wrong for non-equity groups. Migration: use `cfi.decode` and its table-derived predicates — `cfi.voting?` → `cfi.decode.voting?` — which answer only where the group defines the concept. Two names do not carry over name-for-name: migrate `cfi.equity?` to `cfi.category == :equity` (the removed helper was category-wide; the value-level `cfi.decode.equity?` is `true` only when an attribute *value* is `:equity`, e.g. an equity underlying — not for an equity instrument itself), and `cfi.no_restrictions?` to `cfi.decode.free_of_restrictions?` (the ISO value name)
