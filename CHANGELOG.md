...
- **BREAKING (types only):** `Checkable#check_digit`'s RBS signature widened to `Integer | String` to admit DTI's letter check character. Each existing check-digit type's `calculate_check_digit` and class-level `.check_digit` are individually re-narrowed back to `Integer` in their own `.rbs`, but the shared `check_digit` attr_reader itself cannot be narrowed per-subclass (RBS instance-variable types are invariant across a hierarchy), so `ISIN#check_digit`/`CUSIP#check_digit`/`SEDOL#check_digit`/`FIGI#check_digit`/`LEI#check_digit`/`IBAN#check_digit`/`CEI#check_digit` are now statically typed `Integer | String` for Steep/RBS-checked consumers. Runtime values are unaffected — these 7 types always return a plain `Integer` at runtime, as before
