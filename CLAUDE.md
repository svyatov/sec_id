...
- Deprecated method aliases warn then delegate: instance/class `check_digit` → `checksum`, `calculate_check_digit` → `calculate_checksum`, class-level `has_check_digit?` → `has_checksum?`. Written as explicit typed methods (not metaprogrammed) so Steep sees both name sets and `STEEP_UNTYPED_BASELINE` holds.
...
- `components` (and thus `to_h`/`deconstruct_keys`) carries both `:checksum` and a mirrored `:check_digit` key through v7, written as an explicit `check_digit: checksum` value so the deprecated reader never fires internally.
