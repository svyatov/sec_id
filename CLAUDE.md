...
- CFI: strict ISO 10962:2021 validation — `detect_errors` returns `:invalid_category`, `:invalid_group`, and/or `:invalid_attribute` (impermissible attribute letter, `K` code missing `XXXX`, or an `ED` cross-position rule violation); attribute checks are skipped when the group is already invalid. All tables live in `SecID::CFITables` (`lib/sec_id/cfi/tables.rb`); `CATEGORIES`/`GROUPS`/`.categories`/`.groups_for`/`#category`/`#group` are derived from it. `#decode` returns a frozen `CFI::Classification` (`lib/sec_id/cfi/classification.rb`) exposing decoded symbols, ISO labels, table-derived value predicates, `#to_s`, and `#to_h`/`#as_json`; returns `nil` for an invalid CFI. `components`/`to_h` keep raw letters (decoded output is reachable only through `#decode`). The old category-wide equity predicates were removed (migrate to `cfi.decode.<predicate>`)
