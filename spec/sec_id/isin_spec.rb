...
RSpec.describe SecId::ISIN do
  ...
  describe '.valid?' do
    context 'when ISIN is incorrect' do
      it 'returns false' do
        expect(described_class.valid?('US03783315')).to eq(false)
        expect(described_class.valid?('US037833104')).to eq(false)
        expect(described_class.valid?('US0378331004')).to eq(false) # invalid check-digit
        expect(described_class.valid?('US03783315123')).to eq(false)
      end
    end
    ...
    context 'when ISIN is valid' do
      it 'returns true' do
        %w[
          ...
          expect(described_class.valid?(isin_number)).to eq(true)
        end

        expect(described_class.valid?('AU0000XVGZA3')).to eq(true)
        expect(described_class.valid?('AU0000VXGZA3')).to eq(true) # it's not a typo, it's a check-digit flaw in ISIN
      end
    end
  end
  ...
  describe '.valid_format?' do
    context 'when ISIN is incorrect' do
      it 'returns false' do
        expect(described_class.valid_format?('US03783315')).to eq(false)
        expect(described_class.valid_format?('US03783315123')).to eq(false)
      end
    end
    ...
    context 'when ISIN is valid or missing check-digit' do
      it 'returns true' do
        ...
        expect(described_class.valid_format?('US037833104')).to eq(true)
        expect(described_class.valid_format?('US0378331004')).to eq(true)
      end
    end
  end
